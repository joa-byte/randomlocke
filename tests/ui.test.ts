...
class FakeClient extends PokeClient {
  override async catalog(kind: string): Promise<Named[]> {
    return kind === 'pokemon' ? Object.values(species).map(p=>({id:p.id,label:p.label}))
      : kind === 'move' ? Object.keys(moves).map(id=>({id,label:id})) : [{id:'choice-band',label:'Choice Band'}];
  }
  ...
  override async get(path: string) { if (path === 'item/choice-band') return {name:'choice-band'}; throw new ApiError('Objeto inexistente',404); }
}
...
test('DOM + HTTP: create/edit team, reveal rival moves, errors, persistence and removal', async () => {
  ...
  try {
    ...
    click('#add'); input('#pokemon-input','Gastrodon'); await ready(); input('#move-0','earthquake'); await save();
    ...
    input('#move-1','ice-beam'); await ready(); await save();
